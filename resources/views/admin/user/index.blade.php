...
@section('content')
        ...
        <div class=" mt-4">
            <div class="card">
                <div class="card-header text-center font-weight-bold d-flex justify-content-between w-100">
                    <h4 class="my-auto">User Table</h4>
                    <button class="btn btn-sm btn-danger mx-2 d-none" id="deleteAllBtn">Delete All</button>

                    <a href="{{ route('user.create') }}" class="btn btn-sm btn-outline-success ml-auto">Add
                        User/Member</a>
                </div>
                <div class="card-body">
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', 'button#deleteAllBtn', function() {
            ...
            let url = "{{ route('deleteSelectedUser') }}";
            if (checkedData.length > 0) {
                confirm("Are you sure to delete selected User? Action cannot be reverted.")
                $.post(url, {
                    ids: checkedData,
                    "_token": "{{ csrf_token() }}"
                }, function(data) {
